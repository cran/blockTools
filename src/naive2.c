...
void naive2(double *vec, int *n, int *nrow, int *ntr, int *l2, int *l1names,  int *valid, double *validvar, double *validlb, double *validub, int *verbose, double *pairdist, int *result){
  ...
  for(i=0; i<*nrow; i++){
	...
	for(ii = (star[iii] + 1); ii <= *nrow; ii++){
	  if(vec[(tri(ii - 2) + star[iii]) - 1] < min){
	    t=0;
	    min = vec[(tri(ii - 2) + star[iii]) - 1];
	    mn[k-1] = (tri(ii - 2) + star[iii]);
	  }
	  else if (vec[(tri(ii - 2) + star[iii]) - 1] == min){
	    t++;
	  }
	}
	if(t > 0){
      ...
      t=0;
      for(iii=0; iii<k; iii++){
        if(ceil(sqrt(2*mn[k-1]+.25)+0.5) == star[iii]){
	 star[k] = (mn[k-1]-((star[iii]-2)*(star[iii]-1)/2));
	 t++;
	}
      }
      if(t==0){
